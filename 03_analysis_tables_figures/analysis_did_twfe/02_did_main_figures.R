...
p_road <- ggarrange(
  make_roads_fig("dmspols_harmon_ihs",
                 "Dependent Variable: Nighttime Lights") +
    labs(x = NULL),
  
  make_roads_fig("globcover_urban_sum_ihs",
                 "Dependent Variable: Urban") +
    labs(x = NULL),
  
  make_roads_fig("globcover_cropland_sum_ihs",
...
p_ntl <- ggarrange(
  make_ntl_fig("dmspols_harmon_ihs",
                 "Dependent Variable: Nighttime Lights") +
    labs(x = NULL),
  
  make_ntl_fig("globcover_urban_sum_ihs",
                 "Dependent Variable: Urban") +
    labs(x = NULL),
  
  make_ntl_fig("globcover_cropland_sum_ihs",
                 "Dependent Variable: Cropland"),
...
# Export -----------------------------------------------------------------------
ggsave(p_road,
       filename = file.path(paper_figures, "did_main_roads.png"),
       height = 8, width = 8)

ggsave(p_ntl,
       filename = file.path(paper_figures, "did_main_ntl.png"),
       height = 8, width = 8)
