...
for(end_date in c(2009, 2016)){
  ...
  # Regressions ------------------------------------------------------------------
  dmspols_k     <- as.formula(paste("dv_dmspols ~ near_elec_trans | 0 | 0 | ", "cluster_var")) %>% felm(data = df)
  urban_k <- as.formula(paste("dv_gcurban ~ near_elec_trans | 0 | 0 | ", "cluster_var")) %>% felm(data = df)
  crop_k  <- as.formula(paste("dv_gccrop  ~ near_elec_trans | 0 | 0 | ", "cluster_var")) %>% felm(data = df)
  
  dmspols_b_k     <- as.formula(paste("dv_dmspols ~ near_elec_trans + near_elec_transXdmspols_1996_bin4_2 + near_elec_transXdmspols_1996_bin4_3 + near_elec_transXdmspols_1996_bin4_4 | 0 | 0 | ", "cluster_var")) %>% felm(data = df)
  urban_b_k <- as.formula(paste("dv_gcurban ~ near_elec_trans + near_elec_transXdmspols_1996_bin4_2 + near_elec_transXdmspols_1996_bin4_3 + near_elec_transXdmspols_1996_bin4_4 | 0 | 0 | ", "cluster_var")) %>% felm(data = df)
  crop_b_k  <- as.formula(paste("dv_gccrop  ~ near_elec_trans + near_elec_transXdmspols_1996_bin4_2 + near_elec_transXdmspols_1996_bin4_3 + near_elec_transXdmspols_1996_bin4_4 | 0 | 0 | ", "cluster_var")) %>% felm(data = df)
