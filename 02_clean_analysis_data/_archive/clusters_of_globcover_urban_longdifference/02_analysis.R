...
for(theta in c(1,2,5,8)){
  for(log in c("_log")){
    for(exclude100 in c("")){
      ...
      lm_dmspols_zhang_ihs_ADDIS             <- felm(dmspols_zhang_ihs             ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2012)
      lm_dmspols_zhang_sum2_ADDIS             <- felm(dmspols_zhang_sum2            ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2012)
      lm_dmspols_zhang_sum6_ADDIS            <- felm(dmspols_zhang_sum6            ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2012)
      lm_dmspols_zhang_sum0greater_bin_ADDIS <- felm(dmspols_zhang_sum0greater_bin ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2012)
      lm_globcover_urban_sum_ADDIS           <- felm(globcover_urban_sum           ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2016)
      lm_globcover_cropland_sum_ADDIS        <- felm(globcover_cropland_sum        ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2016)
      lm_ndvi_ADDIS                          <- felm(ndvi                          ~ MA_var*distance_city_addisababa_log | 0 | 0 | 0, data = data2016)
