...
for(dataset_type in rev(c("rsdp_rand_treated", "rsdp_rand", "rsdp_rand_restrict", "rsdp"))){
  ...
  determine_distance_to_points <- function(year, points, roads){
    ...
    # points dataframe
    speed_vec <- sort(unique(roads_yyyy[[paste0("Speed", year-1)]]))
    for(speed in speed_vec){
      print(paste(speed, year, "-----------------------------------------------"))
      
      roads_subset <- roads_yyyy[roads_yyyy[[paste0("Speed", year-1)]] %in% speed,] 
      roads_subset <- roads_subset %>% st_as_sf() %>% st_combine() %>% as("Spatial")
      roads_subset$id <- 1
      
      points[[paste0("distance_improvedroad_speedbefore_", speed)]] <- gDistance_chunks(points, 
                                                                                       roads_subset, 
  ...
  points_rsdp1to3 <- lapply(1997:2016, determine_distance_to_points, points, 
    ...
    bind_rows() %>%
    dplyr::rename_at(vars(contains("speedbefore")), . %>% str_replace_all("speedbefore", "speedbefore_p1to3"))
  ...
  points_rsdp4 <- lapply(1997:2016, determine_distance_to_points, points, 
    ...
    bind_rows() %>%
    dplyr::rename_at(vars(contains("speedbefore")), . %>% str_replace_all("speedbefore", "speedbefore_p4"))
  ...
  saveRDS(points_all, file.path(panel_rsdp_imp_dir, DATASET_TYPE, "individual_datasets", 
                                paste0("distance_roads_improved_by_speedlimit_before",out_suffix,".Rds")))
