...
for(year in 1992:2015){
  print(year)
  gc_yyyy <- raster(file.path(data_file_path, "Globcover", "RawData", "1992_2015_data", "ESACCI-LC-L4-LCCS-Map-300m-P1Y-1992_2015-v2.0.7.tif"), band=(year-1991)) %>% crop(extent(woreda))
  coords_urban[[paste0("gc_", year)]] <- raster::extract(gc_yyyy, coords_urban)
}

for(year in 2016:2018){
  print(year)
  gc_yyyy <- raster(file.path(data_file_path, "Globcover", "RawData", "2016_2018_data", paste0("C3S-LC-L4-LCCS-Map-300m-P1Y-",year,"-v2.1.1.tif"))) %>% crop(extent(woreda))
  coords_urban[[paste0("gc_", year)]] <- raster::extract(gc_yyyy, coords_urban)
}
